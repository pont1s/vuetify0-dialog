...
function material(seed, options = {}) {
	if (!HEX_RE.test(seed)) throw new Error(`[@vuetify/v0] Invalid seed color: "${seed}". Expected a hex string (e.g., "#6750A4").`);
	const { variant = "tonalSpot", contrast = 0 } = options;
	const hct = Hct.fromInt(argbFromHex(seed));
	const SchemeClass = VARIANTS[variant];
	if (!SchemeClass) throw new Error(`[@vuetify/v0] Unknown material variant: "${variant}"`);
